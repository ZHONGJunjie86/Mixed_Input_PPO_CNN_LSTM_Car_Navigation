...
//RL-agent
  species bus_agent skills: [advanced_driving,RSkill] { 
	  ...
	  point pre_point;
	  file Rcode_pause<-text_file("D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/R_pause.txt"); //file Rcode_clear<-text_file("D:/Software/GamaWorkspace/Python/R_clear.txt");
      file Rcode_read<-text_file("D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/R_read.txt");
	...
	action reward_calculate{
    	if(time_pass != 0){
    	    ...
    	    if(distance_front_car_before < 30){punishment <- collision*0.1 + jam*0.0001;}
		    if(real_speed<=target_speed and real_speed != 0){ //前面有车 或 出生
		        if(distance_front_car_before < 30 or current_target = targets at 0){
		        	reward <- 0.001*(real_speed/target_speed);
		        }  
			    else{reward <- 0.001 - 0.00066*(target_speed/real_speed);}////+ (time_pass > time_target ? -0.00013 : 0.00005); 
		    }
		    else if(real_speed = 0){reward <- -0.001;}
		    else{ //超速
    ...
    action Python {
    	 save cars_closest_front to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_img/save_NPC_front.csv" type: "csv" header: false;
    	 save cars_closest_behind to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_img/save_NPC_behind.csv" type: "csv" header: false;
    	 save location to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_img/save_self.csv" type: "csv" header: false;
    	 do reward_calculate;
    	 do distance_cal;
    	 ...
    	 save [real_speed/10, target_speed/10, elapsed_time_ratio, distance_left/100,distance_front_car/10,distance_behind_car/10,reward,done,over] //time_pass
    	                                to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/GAMA_intersection_data_1.csv"  type: "csv" header: false; 
    	 save [real_speed/10, target_speed/10, elapsed_time_ratio, distance_left/100,distance_front_car/10,distance_behind_car/10,reward,done,over] 
    	                                to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/GAMA_intersection_data_2.csv"  type: "csv" header: false; 
         //write "read";
	     ...
	     loop while: check_receive = 0 {
	      ...
	      save [0] to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/python_AC_1.csv"  type: "csv" header: false; 
	      save [0] to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_R/python_AC_2.csv"  type: "csv" header: false;
	      jam<-0;
      }    
			        
    reflex collision_judgement when :cycle>=0 and first_time !=1{   
        //找出 bus_closest; bus bus_2nd_closest;
        cars_closest_front <- [];cars_closest_behind <- [];
	        ...
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false){
                ...
                if(current_road != bus_decided.current_road and current_target != bus_decided.current_target ){ //and targets index_of current_target != 0
                    ...
                    if( remain_self = 1 and remain_bus_decided > 1 ){
                        ...
                        if(current_target = next_2_point_bus_closest ){
                    	    if(distance_behind_car=100){distance_behind_car<-Euclidean_distance;}
            	    	    else{distance_behind_car<-Euclidean_distance>distance_behind_car?distance_behind_car:Euclidean_distance;}
            	    	    cars_closest_behind<-cars_closest_behind+bus_decided.location;
                        }
                	...
                	if(remain_self > 1 and remain_bus_decided = 1 ){
            	        ...
            	        if(next_2_point_self = bus_decided.current_target ){
            		        if(distance_front_car=100){distance_front_car<-Euclidean_distance;}
            	    	    else{distance_front_car<-Euclidean_distance>distance_front_car?distance_front_car:Euclidean_distance;}
            	    	    cars_closest_front<-cars_closest_front+bus_decided.location;
                        }
	    ...
	    loop bus_decided over: cars_front_behind{
	        ...
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false){
                ...
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving
                    // agent 在后面，判断追尾 //s=v0t + 1/2at^2
            	    if((self.location distance_to current_target) > (bus_decided.location distance_to bus_decided.current_target)){
            		    if((Euclidean_distance+bus_decided.real_speed) < (real_speed+0.5*acceleration)){collision<-1; write "同一条路上,同向,右行,Collision!"+Euclidean_distance+" "+distance_front_car;}
                    }
                    //agent 在前面，判断堵车
                    else{
                    	if((Euclidean_distance+real_speed+0.5*acceleration) < NPC_speed){jam<-1;write "同一条路上,同向,右行,Jam! "+Euclidean_distance+" "+distance_behind_car;}
                    }
                 }
                //不同路，同向,右行
                if(current_road != bus_decided.current_road and current_target != bus_decided.current_target ){ //and pass_target index_of next_point_self != 0
                	...
                	if(remain_self = 1 and remain_bus_decided > 1 ){
                	    ...
                	    point next_2_point_bus_closest <- bus_decided.targets at next_2_point_index_bus_decided;
                        if(current_target = next_2_point_bus_closest ){
                    	    if((Euclidean_distance+real_speed+0.5*acceleration) < NPC_speed){jam<-1;write "不同路，同向,右行，Jam! "+Euclidean_distance+" "+distance_behind_car;}
                        }
                	}
                	if(remain_self > 1 and remain_bus_decided = 1 ){
                		// agent 在后面，判断追尾
                	    ...
                	    point next_2_point_self <- targets at next_2_point_index_self;
            	        if(next_2_point_self = bus_decided.current_target ){
            		        if((Euclidean_distance+bus_decided.real_speed) < (real_speed+0.5*acceleration)){collision<-1;write "不同路，同向,右行,Collision! "+Euclidean_distance+" "+distance_front_car;} //用欧氏距离粗略估计
                        }
                	}
                	if(remain_self > 1 and remain_bus_decided > 1 ){
                		int next_2_point_index_bus_decided <- bus_decided.targets index_of bus_decided.current_target + 1;
                	    ...
                	    // agent 在后面，判断追尾
            	        if(next_2_point_self = bus_decided.current_target ){
            		        if((Euclidean_distance+bus_decided.real_speed) < (real_speed+0.5*acceleration)){collision<-1;write ">>1不同路，同向,右行,Collision! "+Euclidean_distance+" "+distance_front_car;
            		            //write "不同路，同向,右行"+current_road + bus_decided.current_road + next_point_self + next_point_bus_closest;
            		        } //用欧氏距离粗略估计
                        }
                        //agent 在前面，判断堵车
                        if(current_target = next_2_point_bus_closest ){
                    	    if((Euclidean_distance+real_speed+0.5*acceleration) < NPC_speed){jam<-1;write ">>1不同路，同向,右行,Jam! "+Euclidean_distance+" "+distance_behind_car;
                    	    	 //write "不同路，同向,右行"+current_road + bus_decided.current_road + next_point_self + next_point_bus_closest;
	    ...
	    if(distance_front_car<30){time_target<-time_target+1;}  // or distance_behind_car<20
	...
	reflex time_to_restart when:(distance_left)<=1 or first_time = 1 or collision = 1 {  //a_flag_checked_by_light = 0 and checked = false
		 ...
		 distance_passed <- 0.0;distance_all<- 0.0;reward<-0.0;collision<-0;jam<-0;
		 do road_weight;
         ...
         save [location,route] to: "D:/Software/PythonWork/GAMA_python/PPO_Mixedinput_Navigation_Model/GAMA_img/route.csv" type: "csv" header: false;
         do Python;
	} 
	...
	reflex move when: current_path != nil or a_flag_checked_pass_light != 0 {//道が決まり、目的地が決まれば動く  
	    if (episode<total_episode and done = 0 ){
           ...
           if(real_speed<=0){real_speed <- 0.0;}
        }
		do drive;
